...
## Link Component ou watn

- ### link helps to navigate the pages internally and if you want to naviage to a new page you should use href
...
- give error element and in the error page use the useerrorRoute() in the error page and get the error details from that error object
- bubbles up
...
- colors and bugs
- we can also write inline css in it
...
- all of them are exported as default so we can use any name
...
<!-- by this every route will get the dark theme value as black by default -->
...
- fetch API(experimental) || axios
- global await (top-level await)
- watch mode

```js
//method1--by fetch method in then
fetch(url)
  .then(res => res.json)
  .then(data => cs.log(data));
//method2--by fetch method in fn async await and invoke the fn()
const getdata = async functoin(){
  const res = await fetch(url)
  cosnt data = await res.json()
  console.log(data)
}
//method3--by fetch method in trycatch no need of invoke and async keyword
try {
...
```js - route is wrong and placement is important
app.use('*', (req, res) => {
...
```js - route id not matched or internal code error
...
- try/catch in the controller prevents the server from crash if any error occurs
  server.js
...
-super in class
In JavaScript, the super keyword is used to call the constructor of a parent class when you're working with classes and inheritance.
...
//object.values(iterate and give the array values)
...
export const validateIdParam = withValidationErrors([
  param('id')
    //if the return gives true the id will pass through the middleware or server will send the response back from the validation middleware alone
...
const register = async (req, res) => {
  ...
  //when the user is registerd we no need to send the user detalis as response just send the mesg its is enough
...
```js
//if didnt used this package then it will show the cookie than cookies in req from the browser
...
```js
//giving the value logout to the cookie and ending the expire date by current millisec
...
```js
//by this method in shema we can restrict the property of the model form db to server
...
I apologize for any confusion caused. Based on the original proxy configuration you provided:
...
If your backend URL is `http://localhost:5100/api/users`, then the proxy configuration should indeed work as expected. The `target` value specifies where the proxy should forward the request, and the `rewrite` function is designed to remove the `/api` prefix from the request path before forwarding it.

With the setup above, when your frontend code makes a request to `/api/users`, the following process occurs:

1. The proxy intercepts the request to `/api/users` due to the `/api` prefix match.
2. The `rewrite` function removes the `/api` prefix, resulting in the request being rewritten to `/users`.
3. The proxy forwards the rewritten request to the backend at `http://localhost:5100/api/users`.

So, the URL will still correctly match your backend route at `http://localhost:5100/api/users`.

If you're observing a behavior where the URL is not being correctly rewritten or matched, it might be worth double-checking your proxy configuration and ensuring that there are no typos or conflicts with other settings.
...
Yes, that's correct. If you use only `/users` in your `fetch` request from the frontend, and you have the proxy configuration you provided:
...
The request made by the frontend will be intercepted by the proxy, and the `rewrite` function will be applied. Since the `rewrite` function replaces the `/api` prefix with an empty string, the final URL that will be used for the request will indeed be `http://localhost:5100/api/users`.

So, even if you use `/users` in your `fetch` request, the proxy will still intercept it, rewrite the URL, and forward it to the backend API at `http://localhost:5100/api/users`. This is the behavior of the proxy configuration you've set up.
Yes, that's correct. If you use only `/users` in your `fetch` request from the frontend, and you have the proxy configuration you provided:
...
watch mode -> "watch":"node --watch server.js"
...
//if we use form data api for the key we should give the name property in the input

```js
export const action = async ({ request }) => {
  const formData = await request.formData(); //[....[],[]]
  const data = Object.fromEntries(formData); //{key:value}
  try {
    ...
    >
      <Wrapper>
        <main className='dashboard'>
         ...
            <div className='dashboard-page'>
  {/* in react routert we can pass the data through outlet.
  if want to provide value into the subcomponents/child routes through the parent route we can use the outlet and in the outlet we can pass the context={data} */}
              <Outlet context={{ user }} />
            </div>
          </div>
        </main>
...
//we dont need req as params as axios get data
...
```js
//loader can be used to exract params and data as response from the router
//it is used outside the componenet
export const loader = async ({ params }) => {
  try {
    //this provide data to the loader in route and invoked by useLoader and if it is not desturctured and the invoked useloaderdata function is also not destructured
...
const EditJob = () => {
  //to access the componenet we can use the use params
...
const EditJob = () => {
  //data is already destructured so destruecture the job nested object
  const { job } = useLoaderData();
...
Certainly! This step in the code is responsible for reading a JSON file named 'mockData.json' located in the './utils/' directory relative to the current module file (the file containing this code). Let's break down the code line by line:

1. `new URL('./utils/mockData.json', import.meta.url)`: This part constructs a new URL object. The first argument, './utils/mockData.json', represents the relative path to the JSON file you want to read. The second argument, `import.meta.url`, is a special meta-property in ECMAScript modules that provides the URL of the current module. So, it's essentially creating a URL to the 'mockData.json' file relative to the current module.

2. `await readFile(...)`: This part uses the `readFile` function from the 'fs/promises' module to read the file asynchronously. The `await` keyword is used to wait for the reading operation to complete before proceeding to the next step.

3. `JSON.parse(...)`: After reading the file, the code parses its contents using `JSON.parse`. This function takes a JSON-formatted string and converts it into a JavaScript object. In this case, it's converting the contents of 'mockData.json' into a JavaScript object.

So, in summary, this step fetches the JSON data from the 'mockData.json' file, converts it into a JavaScript object, and assigns it to the variable `jsonJobs`. This variable is then used later in the code for further processing, such as mapping and creating job records in a MongoDB database.
...
```js
export const showStats = async (req, res) => {
  let stats = await Job.aggregate([{ $match: { createdBy: new mongoose.Types.ObjectId(req.user.userId) } }, { $group: { _id: '$jobStatus', count: { $sum: 1 } } }]);
